...
import 'package:animated_fl_chart/main_class/src/chart/base/base_chart/base_chart_painter.dart';
import 'package:animated_fl_chart/main_class/src/extensions/border_extension.dart';
...
abstract class BaseChartData with EquatableMixin {
  ...
  }) : borderData = borderData ?? AFlBorderData();
...
/// Holds data to drawing border around the chart.
class AFlBorderData with EquatableMixin {
  ...
  /// Lerps a [AFlBorderData] based on [t] value, check [Tween.lerp].
  static AFlBorderData lerp(AFlBorderData a, AFlBorderData b, double t) {
    return AFlBorderData(
      show: b.show,
      border: Border.lerp(a.border, b.border, t),
    );
  }

  /// Copies current [AFlBorderData] to a new [AFlBorderData],
  /// and replaces provided values.
  AFlBorderData copyWith({
    bool? show,
    Border? border,
  }) {
    return AFlBorderData(
...
abstract class FlTouchData<R extends BaseTouchResponse> with EquatableMixin {
  ...
  /// based on the provided [AFlTouchEvent] and [BaseTouchResponse]
...
/// It gives you the happened [AFlTouchEvent] and existed [R] data at the event's location,
